                                                  ...
                                                  const ViewLayer &view_layer)
{
  ...
  /* Nuclear: does a mask list reference `mask_layer`, directly (leaf name) or via a group/peg
   * whose descendant leaves include it? Cross-object mattes are handled by the cross-object path.
   */
...
void Instance::draw_mask(View &view, tObject *ob, tLayer *layer)
{
  ...
  /* Nuclear: cross-object mattes. Render the matte object's (cached) layer passes into the same
   * mask buffer. Limitation: the matte object must be evaluated/visible in the view layer to be
   * cached; a matte object that was never synced resolves to an empty silhouette. */
  for (const tMatteRef &matte : layer->mattes) {
    ...
    /* Optional node filter: empty = whole object; a layer name = that layer; a group name = all
