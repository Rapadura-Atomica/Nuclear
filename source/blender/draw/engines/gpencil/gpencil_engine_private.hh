...
struct Instance final : public DrawEngine {
  ...
  /* Nuclear: map from each synced (evaluated) object to its tObject, so cross-object mattes can
   * find the matte object's already-built layer passes at draw time. Rebuilt every sync. */
