"""A bancada do artista atravessa a troca de take.
...
A anotação vai para `workspace.json`, dentro do board. No board e não na máquina
porque a paleta é do episódio; em arquivo próprio e não no `project.json` porque
isto é bancada, não conteúdo — o índice do board não deve mudar de checksum
porque alguém engrossou o traço.
...
def restore_brushes_soon(data: dict) -> bool:
    ...
    """
    if bpy.app.background or not (data or {}).get("brushes"):
        return False
...
def save(store, data: dict) -> bool:
    """Grava a bancada no board. Falha de escrita não derruba o salvar do take."""
    try:
        bench_path(store).write_text(
...
def load(store) -> dict:
    try:
        dados = json.loads(bench_path(store).read_text(encoding="utf-8"))
