...
#   - Telemetria de presenca + relatorio de falha ficam LIGADAS por padrao, SEM
#     tela de consentimento. Opt-out via --no-telemetry (ou removendo o prefixo
#     'env NUCLEAR_TELEMETRY_OFF=1 NUCLEAR_CRASH_OFF=1' do Exec do .desktop).
...
#     --no-telemetry  ja instala com telemetria + crash report desligados.
...
usage() {
    # Imprime o cabecalho em comentario (do shebang ate a 1a linha nao-comentario),
    # sem depender de numeros de linha fixos (o artefato dist tem banner extra).
...
# Rede de seguranca para o uso "duplo-clique num executavel": se caimos no backend
# de texto (sem kdialog/zenity/whiptail) MAS nao ha terminal, um duplo-clique nao
# teria como interagir. Numa sessao grafica, relanca a si mesmo dentro de um
# emulador de terminal para o wizard-texto ter um tty. So age nesse caso-limite.
_reexec_in_terminal_if_needed() {
    [ "$UI" = "text" ] || return 0
    [ "$ASSUME_YES" = "1" ] && return 0         # nao-interativo: nao precisa de tty
    [ -n "${NUCLEAR_INSTALLER_UI:-}" ] && return 0  # backend forcado: respeita a escolha
    [ -t 0 ] && return 0                        # ja temos terminal
    [ -n "${NUCLEAR_REEXEC:-}" ] && return 0    # evita loop de relançamento
    _in_gui_session || return 0                 # sem GUI e sem tty: nada a fazer
    local term
    for term in konsole gnome-terminal xfce4-terminal alacritty kitty xterm x-terminal-emulator; do
        _has "$term" || continue
        case "$term" in
            gnome-terminal) NUCLEAR_REEXEC=1 setsid "$term" -- bash "$0" >/dev/null 2>&1 & ;;
            *)              NUCLEAR_REEXEC=1 setsid "$term" -e bash "$0" >/dev/null 2>&1 & ;;
        esac
        exit 0
    done
    return 0   # nenhum terminal encontrado: segue no texto (melhor esforco)
}
...
EOF
    ...
    # Texto amigavel do que esta sendo instalado (ex.: "Nuclear 1.4.4 (Beta)").
...
main() {
    ...
    # 1. Pre-checagem de ferramentas e permissao (silenciosa; so erra se faltar algo).
    preflight

    # 2. Ler manifesto e checar espaco.
    read_manifest
    check_space

    # 3. Tela unica e clara: O QUE + ONDE + confirmar.
    #    Instala SEMPRE na home do usuario ($HOME/Nuclear, ex.: /var/home/<user>/Nuclear).
    #    Para automacao, --dir troca o destino sem perguntar.
    local size_mb="?"
    [ "$M_SIZE" -gt 0 ] 2>/dev/null && size_mb=$(awk "BEGIN{printf \"%.0f\", $M_SIZE/1048576}")
    ui_confirm "$APP_TITLE" "Voce esta instalando:

  $M_VERSTRING
  Um aplicativo de animacao 2D.
...
Instalar agora?" || { ui_info "$APP_TITLE" "Instalacao cancelada."; exit 0; }
    ...
    # 3.5. Telemetria de presenca + relatorio de falha ficam LIGADAS por padrao,
    #      sem tela de consentimento (a pergunta foi retirada). O opt-out honesto
    #      continua: instalar com --no-telemetry (ou tirar o prefixo 'env NUCLEAR_...'
    #      do atalho depois) desliga as duas camadas via env vars no Exec do .desktop.

    # 4. Download + verificacao + extracao + layout versionado.
    do_install

    # 5. Symlink 'current' + atalho + MIME (telemetria on, salvo --no-telemetry).
    local exec_prefix=""
