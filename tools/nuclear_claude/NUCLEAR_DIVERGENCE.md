...
- `scripts/startup/nuclear_clear_camera.py` — **Clear Camera** (item em *View > Viewpoint*, operador `scene.nuclear_clear_camera`): limpa a cena ativa de volta ao template recém-aberto — apaga TODOS os objetos (desenhos, meshes, rigs, luzes) com sua animação, as collections que os continham e os markers, purga os órfãos e reconstrói a câmera padrão do template (0,-12,0 / rot X 90° / 50 mm / clip 1000) numa collection "Camera". Apaga pela API de dados (não por `object.delete`), então objeto escondido/travado/não-selecionável vai junto; **só a cena ativa** — um objeto também linkado em outra cena (take de storyboard) é apenas desligado desta. Sai do modo Draw/Edit antes de apagar (o template abre em Draw). Menu injetado por `append` em `VIEW3D_MT_view_viewpoint`, **sem tocar `space_view3d.py`**. Confirmação obrigatória (`invoke_confirm`) e undo validado na GUI. Selftest: `tools/nuclear_scene/selftest_clear_camera.py` (19/19). **Não cria ponto quente na §2.**
