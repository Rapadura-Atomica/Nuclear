...
- [feito] **Rename do executável e artefatos `blender*` → `nuclear*` (2026-07-08).** O que era o item "Deferido" do rebrand da 1.5.0. Binário instalado agora é **`nuclear`** (o *target* CMake continua `blender` — só `OUTPUT_NAME` muda, minimiza divergência).
  ...
  - **Deferidos conscientes:** bundle macOS `Blender.app` (sem build p/ validar; deploy é Linux), keyconfig "Blender" (nome persistido nas prefs, exige migração + default C), env vars `BLENDER_USER_*`/`BLENDER_SYSTEM_*` (superfície programática, tipo `bpy`), domínio gettext `blender.mo`, `snap/` (não distribuído), `share/blender` do install não-portátil, appName Vulkan/XR (nota acima).
